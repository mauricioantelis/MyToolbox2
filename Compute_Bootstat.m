...
%function [accarrayout, Rbootout, Rbootout2] = Compute_Bootstat(oriargs, formula, varargin)
function [accarrayout, Rbootout, Rbootout2] = Compute_Bootstat(oriargs,cfg)
...
g.boottype    = cfg.boottype;
g.label       = cfg.label;
g.bootside    = cfg.boottype;
g.naccu       = cfg.naccu;
g.basevect    = cfg.baselntmp;
g.alpha       = cfg.alpha;
g.dimaccu     = cfg.dimaccu;
g.dims        = [];
g.shufflemode = 'swap';
g.randmode    = 'opposite';
g.shuffledim  = [];
...
elseif strcmpi(g.boottype, 'rand') & strcmpi(g.randmode, 'inverse')
    ...
    fprintf('Processing permutation statistics for %s (naccu=%d): \n', g.label, g.naccu);
